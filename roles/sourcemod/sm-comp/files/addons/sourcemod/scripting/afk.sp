...
public OnPluginStart() {
	...
	g_hCvarMaxAfkTime = CreateConVar("afk_time", "20", "Number of seconds you can stand still before being marked as AFK.", FCVAR_PLUGIN, true, 5.0);
	g_fMaxAfkTime = GetConVarFloat(g_hCvarMaxAfkTime);
	HookConVarChange(g_hCvarMaxAfkTime, CvarChange_MaxAfkTime);
	
	g_hCvarMinPlayers = CreateConVar("afk_minplayers", "8", "Minimum number of players on the server before the plugin is active.", FCVAR_PLUGIN);
